...
std::map<std::string, std::string> topicReverseSuffixMap;
...
void CallBack::message_arrived(mqtt::const_message_ptr msg)
{
	...
	*/
	...
	if (requestResponseTopicMap.contains(topicPrefix))
	{
		...
		std::string responseTopic = responsePrefix + topicReverseSuffixMap[topic];
		...
		pubmsg->set_qos(qos);
		cli.publish(pubmsg);
		PLOG_DEBUG << "Message published" << "\ttopic: '" << responseTopic << "\tpayload: '" << responseData;
...
void CallBack::connected(const std::string& cause)
{
	...
	nlohmann::json opcuaNodes = config.config["opcua_nodes"];
	PLOG_DEBUG << "Mqtt Subscribe topic:";
	for (auto& data : subscribeMethods.items())
	{
		...
		topicReverseSuffixMap.insert(std::pair<std::string, std::string>(topic, devId));

		for (int i = 0; i < opcuaNodes.size(); i++) {
			std::string nodename = opcuaNodes[i]["Identifier"];
			std::string opc_topic = topicPrefix + devId + "/" +nodename;
			cli.subscribe(opc_topic, qos, nullptr, subListener);
			topicMap.insert(std::pair<std::string, std::string>(opc_topic, topicPrefix));
			topicReverseSuffixMap.insert(std::pair<std::string, std::string>(opc_topic, devId + "/" + nodename));
			std::cout << "key: " << opc_topic << ", value:" << qos << '\n';
			PLOG_DEBUG << "key: " << opc_topic << ", value:" << qos << '\n';
		}
