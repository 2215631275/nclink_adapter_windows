...
static void ndpi_search_ethernet_ip(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  if(packet->tcp != NULL) {
    ...
    NDPI_LOG_DBG2(ndpi_struct, "calculating ETHERNET_IP over udp\n");
  
    if(packet->payload_packet_len >= 8) {
      u_int16_t eth_ip_port = ntohs(2222);

      if((packet->udp->source == eth_ip_port) || (packet->udp->dest == eth_ip_port)) {
	//u_int16_t len = le16toh(*((u_int16_t *)&packet->payload[2])); /* Little endian */
