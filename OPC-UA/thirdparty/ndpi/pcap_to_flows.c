...
static char valid_labels[][32]={/*"Modbus","FINS","EthernetIP","s7comm"*/};  //用于过滤的白名单，可自行添加
static char invalid_labels[][32]={};  //用于过滤的黑名单，可自行添加
...
static void printFlow(struct ndpi_flow *flow, FILE *file) {
    ...
    //流量采集输出内容：src/dst_ip/port、TCP/UDP、持续时间、字节量、内部到达时间、包长度、速率、协议名称、数量
    //fprintf(file, "%s,%u,%s,%u,%s,%.6f,%u,%f,%.6f,%.6f,%u,%u,%u,%.6f,%.6f,%s,%u\n",
    //fprintf(file, "%15s:%-5u,%15s:%-5u,%9s,%13u,%10u,%7u,%7u,%7u,%24.6f,%24.6f,%16s,%7u\n",
    ...
    //csv格式
    // fprintf(file, "%15s:%-5u,%15s:%-5u,%d-%2d-%2d-%2d:%2d:%2d,%d-%2d-%2d-%2d:%2d:%2d,%16.6f,%10u,%16s\n",
...
static void printResults(void) {
    ...
    //fputs("      source_ipport  ,        dest_ipport  ,IP4_proto,flow_duration,flow_bytes,avg_len,min_len,max_len,                 pkt_vel,                byte_vel,        protocol,app_num\n", flow_info_file);
    //fputs("        source_ipport,          dest_ipport,         first_time,          last_time,flow_duration(s),flow_bytes,        protocol\n", flow_info_file);
    fprintf(flow_info_file, "{\n  \"FlowCollection\": [\n");
    comma_flag = 0;
...
//    int op =data->op;
	...
	printf("current net dev name:\n");     
	for (pcap_if_t *pdev = allDev; pdev; pdev=pdev->next)
	{
	    //printf("%s\n",pdev->name);
	    strcpy(devs_name[devs_num],pdev->name);
        // strcpy(devs_disc[devs_num],pdev->src_ip);
        printf("%d.%s\n",devs_num+1,pdev->description);
	    devs_num++;
	}
   ...
   //  out_pcap = pcap_dump_open(handler,file_path); 
    //  if(pcap_loop(handler, -1, pcap_callback, (u_char*)out_pcap) == -1){
    ...
    //if(argc<2) flow_info_file_name="test.csv";
    //else flow_info_file_name = argv[1];
