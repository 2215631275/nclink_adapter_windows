 ...
 * @param dev_uuid  �豸id
*/
FlowCollection::FlowCollection(nlohmann::json sampleItem, std::string dev_uuid, std::int32_t sampleinterval, std::int32_t uploadinterval, mqtt::async_client& cli) :client(cli)
{
...
#else
        std::ifstream file("../thirdparty/ndpi/test.json");
#endif
...
void FlowCollection::UploadTask(void* arg) {
    ...
    while (1) {
        ...
        // modelFile.close();
        //pThread->responseJson["CollectTime"] = pThread->beginTime;
